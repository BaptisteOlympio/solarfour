...
class UnitVectorTest {
    ...
    void shouldHaveUnitNorm_whenCreatedFromAngles() {
        UnitVector v = UnitVector.fromAngles(45, 90);
    ...
    void shouldPointInCardinalDirection(
        double azimuth, double altitude,
        double expectedX, double expectedY, double expectedZ
    ) {
        UnitVector v = UnitVector.fromAngles(altitude, azimuth);
        assertEquals(expectedX, v.getX(), DELTA, "X component mismatch for azimuth=" + azimuth + ", altitude=" + altitude);
        assertEquals(expectedY, v.getY(), DELTA, "Y component mismatch for azimuth=" + azimuth + ", altitude=" + altitude);
        assertEquals(expectedZ, v.getZ(), DELTA, "Z component mismatch for azimuth=" + azimuth + ", altitude=" + altitude);
    }
